...
<p align="center">
  <strong>面向 JD 场景的桌面 Agent 工作台，用于承载生产力与自动化任务</strong>
</p>
...
**JdiClaw** 是一个面向 JD 场景的桌面 Agent 工作台，聚焦在受监督的生产力与自动化流程，例如数据分析、文档撰写、演示材料整理、信息收集、定时跟进等日常桌面任务。
...
- **桌面 Agent 工作台** — 支持数据分析、PPT 制作、视频生成、文档撰写、Web 搜索、邮件收发等受监督的桌面工作流
...
JdiClaw 可以使用 [OpenClaw](https://github.com/openclaw/openclaw) 作为 Agent 引擎。
...
该运行时以解释器为主，不预装 JdiClaw 技能所需的 Python 三方包；相关依赖可在运行时按需安装。
...
JdiClaw 采用 Electron 严格进程隔离架构，所有跨进程通信通过 IPC 完成。
...
Cowork 是 JdiClaw 的核心功能之一 —— 基于 Claude Agent SDK 的 AI 工作会话系统。它面向办公场景设计，能够自主完成数据分析、文档生成、信息检索等复杂任务。
...
JdiClaw 内置 16 种技能，覆盖办公、创作、自动化等多种场景，通过 `SKILLs/skills.config.json` 配置启停和排序：
...
JdiClaw 支持创建定时任务，让 Agent 按计划自动执行重复性工作。
...
JdiClaw 支持将 Agent 桥接到多种 IM 平台。在手机上通过 IM 发送消息即可远程触发桌面端的 Agent 执行任务，让同一套受监督工作流可以延伸到移动端。
...
JdiClaw 内置记忆系统，能够跨会话记住你的个人信息和偏好，让 Agent 越用越懂你。

### 记忆获取方式

- **自动提取** — 对话过程中，系统自动识别并记录你的个人信息（姓名、职业等）、偏好习惯（喜好的语言、格式、风格）和个人事实（养的宠物、使用的工具等），无需手动操作
- **主动告知** — 在对话中直接说「记住我喜欢用 Markdown 格式」「记下我的项目叫 JdiClaw」等，Agent 会以更高置信度存储
...
JdiClaw 将 OpenClaw 依赖锁定到指定的 release 版本，在 `package.json` 中声明：
