 ...
 * and waits for the user's response via an HTTP callback to JdiClaw.
 *
 * This enables delete-confirmation modals on the JdiClaw desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for JdiClaw desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerTool((ctx) => {
      // Only enable for JdiClaw desktop sessions (sessionKey starts with 'agent:main:jdiclaw:').
      ...
      const isLocalDesktop = sessionKey.startsWith('agent:main:jdiclaw:');
      if (!isLocalDesktop) {
        return null;
      }
