...
function tryJdiClawServerFallback(modelId?: string): MatchedProvider | null {
  ...
  console.log('[ClaudeSettings] jdiclaw-server fallback activated:', { baseURL, modelId: effectiveModelId, supportsImage: cachedMeta?.supportsImage });
  return {
...
function resolveMatchedProvider(appConfig: AppConfig): { matched: MatchedProvider | null; error?: string } {
  ...
  if (!modelId) {
    ...
    if (!fallback) {
      const serverFallback = tryJdiClawServerFallback(configuredModelId);
  ...
  // Handle jdiclaw-server provider: dynamically construct from auth tokens
  if (preferredProviderName === ProviderName.JdiClawServer) {
    const serverMatch = tryJdiClawServerFallback(modelId);
    if (serverMatch) {
  ...
  if (!providerEntry) {
    ...
    if (fallback) {
      ...
      const serverFallback = tryJdiClawServerFallback(modelId);
      if (serverFallback) return { matched: serverFallback };
...
export function resolveCurrentApiConfig(target: OpenAICompatProxyTarget = 'local'): ApiConfigResolution {
  ...
  const effectiveApiKey = resolvedApiKey
    || (!providerRequiresApiKey(matched.providerName) ? 'sk-jdiclaw-local' : '');
  ...
  return {
    config: {
      apiKey: resolvedApiKey || 'jdiclaw-openai-compat',
...
/**
 * Map JdiClaw model IDs to OAuth endpoint model names
...
export function resolveAllProviderApiKeys(): Record<string, string> {
  ...
  // jdiclaw-server token is now managed by the token proxy
  // (openclawTokenProxy.ts) — no longer injected as an env var.

    // jdiclaw-server: uses auth accessToken
    const tokens = authTokensGetter?.();
    const serverBaseUrl = serverBaseUrlGetter?.();
    ...
    for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
      ...
      result[envName] = apiKey || 'sk-jdiclaw-local';
    }
...
export function resolveAllEnabledProviderConfigs(): ProviderRawConfig[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(appConfig.providers)) {
    if (!providerConfig?.enabled) continue;
    if (providerName === ProviderName.JdiClawServer) continue;
    ...
    result.push({
      providerName,
      baseURL: effectiveBaseURL,
      apiKey: apiKey || 'sk-jdiclaw-local',
      apiType: effectiveApiFormat === 'anthropic' ? 'anthropic' : 'openai',
